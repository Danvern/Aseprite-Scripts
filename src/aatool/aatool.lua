...
function cutCorners()
		...
		if exploitedPixels[driver[1] * rectangle.height + driver[2]] == nil then
			...
			if spinDirection ~= 0 then
				repeat
					...
					table.insert(strand, { ["x"] = driver[1], ["y"] = driver[2] })
					markPixel()
					table.insert(borderWeb,
						{ ["components"] = strand, ["normalFacing"] = rotateFacing(facing, spinDirection * -2), ["spin"] =
						spinDirection })
	...
	function calculatePixel(point, strand, index, primaryVertexOffset, scale)
		...
		directionsX[rotateFacing(strand.normalFacing, normalOffset * strand.spin)]
		pixel.compareY = strand.components[cornerIndex].y +
		directionsY[rotateFacing(strand.normalFacing, normalOffset * strand.spin)]
